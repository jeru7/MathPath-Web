...
export default function AddStudent({ setShowForm, navigate, initialMode }: IAddStudentProps): ReactElement {
  const [showInitialPrompt, setShowInitialPrompt] = useState<boolean>(!initialMode)
  const [showGenerateCode, setShowGenerateCode] = useState<boolean>(initialMode === "generate")
  const [showManualAdd, setShowManualAdd] = useState<boolean>(initialMode === "manual")
  ...
  return <div className="bg-[var(--primary-black)]/20 fixed left-0 top-0 z-10 flex h-full w-full items-center justify-center">
    ...
    {showInitialPrompt && (
      <div className="relative flex flex-col gap-4 rounded-md bg-[var(--primary-white)] p-8">
        ...
        <div className="flex flex-col gap-2">
          <p className="text-xs font-medium">How do you want to add student?</p>
    ...
    {/* Manual Adding */}
    {showManualAdd && (
      <ManualAdd handleBack={handleBack} />
    )}

    {/* Generate Code */}
    {showGenerateCode && (
      <GenerateCode handleBack={handleBack} />
    )}
  </div>
}
