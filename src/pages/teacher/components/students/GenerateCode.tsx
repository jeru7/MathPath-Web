...
export default function GenerateCode({ handleBack }: IGenerateCodeProps): ReactElement {
  const navigate = useNavigate();
  const [numberOfStudents, setNumberOfStudents] = useState<number>(10)

  const handleNumberOfStudentsInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  return <form>
    <div className="relative flex flex-col gap-2 rounded-md bg-[var(--primary-white)] p-8">
      ...
      {/* Header */}
      <header>
        <h3 className="border-b border-b-[var(--primary-gray)] pb-2 text-2xl font-bold">Add Student - Generate Code</h3>
      </header>
