...
import { CreateAssessmentDTO } from "../../types/assessment/assessment.schema";

export const useAddAssessment = (teacherId: string) => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (newAssessment: CreateAssessmentDTO) => {
      return postData<Assessment, CreateAssessmentDTO>(
